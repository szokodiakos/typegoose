...
const baseProp = (rawOptions, Type, target, key, isArray = false) => {
    ...
    if (rawOptions.mixed) {
        data_1.schema[name][key] = mongoose_1.Mixed;
        return;
    }
    if (isGetterSetter) {
    ...
    if (ref) {
        data_1.schema[name][key] = Object.assign({}, data_1.schema[name][key], { type: mongoose_1.ObjectId, ref: ref.name });
        return;
    }
    const itemsRef = rawOptions.itemsRef;
    if (itemsRef) {
        data_1.schema[name][key][0] = Object.assign({}, data_1.schema[name][key][0], { type: mongoose_1.ObjectId, ref: itemsRef.name });
        return;
    }
    ...
    if (!enumOption) {
        ...
        subSchema = data_1.schema[instance.constructor.name];
    ...
    if (utils_1.isPrimitive(Type) || enumOption) {
        if (isArray) {
